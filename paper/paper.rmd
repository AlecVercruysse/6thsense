...
library(gridExtra)
library(latex2exp)
opts_chunk$set(echo = F, warning = F, fig.align='center', fig.pos = 'h')
...
```{r NASA_image, fig.cap="\\label{fig:figs} NASA Empirical Models for Temperatur and Pressure"}
...
```{r cutdown_diagram, fig.cap="\\label{fig:figs} Cutdown mechanism circuit design"}
...
```{r simple_circuit, fig.cap="\\label{fig:figs} A simple example circuit"}
...
```{r vernier_calibration, echo=F, fig.align='left', fig.cap="\\label{fig:figs}"}
...
```{r vernier_cal2, fig.height=3.5, warning=F, fig.cap="\\label{fig:figs}", fig.pos='h'}
...
```{r bme, fig.height=4, echo=F, fig.cap="\\label{fig:figs}"}
...
```{r bme_temp, fig.height=4, fig.cap="\\label{fig:figs}"}
...
```{r imagebreaka, out.height="100px", fig.cap="\\label{fig:figs} APRS antenna touches the surface of teh balloon in the lower middle section of the image."}
include_graphics("assets/IMAGEBREAKA.png")
```

```{r imagebreakb, out.height="100px", fig.cap="\\label{fig:figs} APRS antenna applies more pressure on the surface of the balloon."}
include_graphics("assets/IMAGEBREAKB.png")
```

```{r imagebreakc, out.height="100px", fig.cap="\\label{fig:figs} Pressure between APRS antenna and balloon is released. A puncture is visible in the balloon."}
...
```{r imagebreakd, out.height="100px", fig.cap="\\label{fig:figs} Reflection of balloon in payload on lower left side of photo."}
...
```{r temp_altitude, fig.cap="\\label{fig:figs}"}
...
As discussed, the launch can be considered a technical success. With the payload and most data recovered, atmospheric analysis has been made possible by 6thsense's launch. The results discussed above agree with the scientific community's current understanding of the atmosphere and atmospheric layers. Not only was data recorded to study the atmosphere, however, but 6thsense's balloon launch was also a test of systems that the team designed to meet the technical challenges of sending equipment to the edge of space.

The 6thsense team, working together for the first time on such a large project, grew not only their knowledge of the subject matter required to develop and launch such a payload, but also communication and collaboration skills, necessary in any large scall venture.