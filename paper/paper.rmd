...
pv$upper_temp_error = sapply(pv$temp_recal, function(x) x + 0.014598 * x)
pv$lower_temp_error = sapply(pv$temp_recal, function(x) x - 0.014598 * x)
...
a <- ggplot(pv, aes(x=height, y=temp_recal)) + geom_point(aes(color="Measured Inside")) + 
  ...
  geom_errorbar(data=pv, aes(x=height, ymin=lower_temp_error, ymax=upper_temp_error))
a
```
Figure 15
